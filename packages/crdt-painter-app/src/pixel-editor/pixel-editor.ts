import { PixelData, RGB } from "./pixel-data";
...
export class PixelEditor {
  ...
  #listener: (state: PixelData["state"]) => void = () => {};
  ...
  async #draw() {
    ...
    for (let row = 0; row < this.#artboard.h; row++) {
      ...
      for (let col = 0; col < this.#artboard.w; col++) {
        ...
        const rgb = this.#data.get(col, row);
        const hexToNumber = (str: string) => parseInt(str, 16);
        const r = hexToNumber(rgb.slice(0, 2));
        const g = hexToNumber(rgb.slice(2, 4));
        const b = hexToNumber(rgb.slice(4, 6));
        buffer[offset] = r;
        buffer[offset + 1] = g;
        buffer[offset + 2] = b;
        buffer[offset + 3] = 255;
